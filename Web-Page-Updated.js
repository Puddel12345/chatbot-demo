...
interface Message {
  ...
  timestamp: Date;
  reactions?: string[];
  thinking?: string; // Add thinking content from Claude
}
...
const ChatPage: React.FC<ChatPageProps> = ({ onBack }) => {
  ...
  const WEBHOOK_URL = 'https://chatbot-demo-production-6c16.up.railway.app';
  ...
  // Function to send message to Railway webhook and get Jo's response
  const getJoResponse = async (userMessage: string): Promise<{ content: string; thinking?: string }> => {
    try {
      const response = await fetch(`${WEBHOOK_URL}/chat`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          // Optional: Add system prompt override if needed
      ...
      console.log('Railway webhook response:', data); // For debugging
      
      return {
        content: data.response || 'Entschuldigung, ich konnte deine Nachricht nicht verarbeiten. Kannst du es nochmal versuchen?',
        thinking: data.thinking // Include thinking content if available
      };
      
    } catch (error) {
      console.error('Railway webhook request failed:', error);
      return {
  ...
  // Alternative function for streaming responses (SSE)
  const getJoResponseStreaming = async (userMessage: string): Promise<{ content: string; thinking?: string }> => {
    try {
            ...
            if (line.startsWith('data: ')) {
              try {
                ...
                if (data.type === 'text') {
                  ...
                  // You could update the UI in real-time here
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // Use regular webhook (faster for UI)
      // For streaming, change to: getJoResponseStreaming(messageToSend)
      const joResponseData = await getJoResponse(messageToSend);
      ...
      setTimeout(() => {
        const joResponse: Message = {
          ...
          sender: 'jo',
          timestamp: new Date(),
          thinking: joResponseData.thinking, // Store thinking content
        };
  ...
  // Function to toggle thinking display (optional feature)
          ...
          <div className="bg-white px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Jo Wünsche • Claude Opus 4</p>
              </div>
            </div>
          </div>
